function SpaceShip(img){
	...
	this.update = function (context, timediff, timestamp)
	{
        ...
        if (this.warp){
            warpWalls.call(this, context);
        }
	};
    this.warp = true;
    var warpWalls = function(context){
        ...
        if (p.x < (0 - width / 2)){
            p.x = p.x + bounds.x + width;
        } else if (p.x > (bounds.x + width / 2)) {
            p.x = p.x - (bounds.x + width);
        }

        if (p.y < (0 - height / 2)){
            p.y = p.y + bounds.y + height;
        } else if (p.y > (bounds.y + height / 2)) {
            p.y = p.y - (bounds.y + height);
        }
